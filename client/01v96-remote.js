...
var remoteApp = {
	...
	bindGlobalEventHandlers: function() {
		...
		// tab navigation: abstract with Hammer.JS
		$navi.hammer({
			hold: false,
			release: false,
			swipe: false,
	...
	bindDynamicEventHandlers: function() {
		...
		// on-buttons
		[].forEach.call(document.querySelectorAll('.on-button'), function(el) {
			el.addEventListener('touchstart', function(e) {
				e.preventDefault();
				e.stopPropagation();
				
				app.eventAbstraction.onButton($(el).parents('.control'));
			}, false);
		});
	},
